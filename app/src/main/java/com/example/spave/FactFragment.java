...
public class FactFragment extends Fragment {
    ...
    static String facts[] = {
            ...
            "There are over 1.1 billion $2 US bills still around",
            ...
            "$20 bills are the most counterfeited bill",
    ...
    // METHOD: saving status of viewed facts
    public void saveFact(){
        SharedPreferences sharedPreferences = getActivity().getSharedPreferences("NUMBER_VALUES", Context.MODE_PRIVATE);
        try {
            sharedPreferences.edit().putString("numbers", ObjectSerializer.serialize(numbers)).apply();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            storedData = (ArrayList<String>) ObjectSerializer.deserialize(sharedPreferences.getString("numbers", ObjectSerializer.serialize(new ArrayList<String>())));
        } catch (IOException e) {
            e.printStackTrace();
        }
//        Toast.makeText(getActivity().getApplicationContext(), storedData.toString(), Toast.LENGTH_LONG).show();
    }
